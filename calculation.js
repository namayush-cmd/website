...
function calculateAll() {
    ...
    // Utilization (Recurring + Non-Recurring)
    let totalUtil =
        getVal("ct1") + getVal("ct2") +
        getVal("ct5") + getVal("ct6");

    // Refund (Recurring + Non-Recurring)
    let totalRefund =
    ...
    let totalUtil_row =
        getVal("t1") + getVal("t11") + getVal("t21") +
        getVal("t31") + getVal("t41") + getVal("t51");

    if (totalUtil !== totalUtil_row) {
        console.warn("⚠ Util mismatch", totalUtil, totalUtil_row);
    }
